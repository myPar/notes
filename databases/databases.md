...
* __Проекция__ - операция $\rho_{A_{i1},..A_{i_k}}(R)$ где $R=R(A_1, ..., A_n)$ отношение, порождающая новое отношение $R_1$ с атрибутами $A_{i1},..A_{i_k}$ из отношения $R$, где $A_{i1},..A_{i_k} \sub A_1,..,A_n$. Отношение $R_1$ порождается следующим образом:
