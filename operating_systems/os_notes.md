...
Каждый жёсткий диск состоит из разделов (_partitions_), расположение и размер которых определяется при форматировании диска. В UNIX разделы являются независимыми устройствами, обращение к которым происходит как к различным носителям данных.  

Разделы могут иметь различные ФС, но на каждом из них может быть только одна, а также одна ФС не может располагаться на нескольких разделах. Возможно конфигурация диска, при которой он будет иметь один раздел с одной ФС.  
...
* Список номеров свободных _inode_ (список индексных дескрипторов).  
