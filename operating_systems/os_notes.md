...
Рассмотрим поля дескрипторов сегментов кода данных и стека:
...
Вычисление физического адреса при страничной реализации виртуальной памяти имеет больше этапов, чем в случае с сегментами. Также вычисляется линейный адрес, посредством сложения адреса начала сегмента и смещения внутри него, только теперь это не физический, а по прежнему логический адрес, который идёт на обработку блоку страничной трансляции процессора.  
...
Сделать это можно при помощи системного вызова _mmap(2)_. Самостоятельно выделять память для отображения (при помощи malloc) не требуется, _mmap(2)_ делает это автоматически в динамических сегментах адресного пространства процесса.
...
_oflag_ - режим открытия файла. Допустимые режимы определяются следующими константами, которые определены в файле _<fcntl.h>_:  
...
#### Что происходит "под капотом" _open(2)_?
...
Чтение из файла осуществляется системным вызовом _read(2)_.
...
Поскольку _read(2)_ и _write(2)_ являются системными вызовами, то их использование влечёт соответствующие накладные расходы при обращении к ядру ОС. Их целесообразно использовать при чтении/записи большого объёма данных. В остальных случаях следует использовать ф-ции буфферезированного ввода-вывода.  
...
_writefs_ - ножество дескрипторов, проверяемых на готовность к записи. Регулярные файлы по умолчанию считаются готовыми к записи. Готовновность операции _connect(SOCKET)_ для сокета в неблокирующемся режиме также является готовностью к записи.  
...
_poll(2)_ делает приблизительно то же, что и _select(2)_, но предоставляет несколько более удобный механизм информирования о том, какие дескрипторы и операции на них его интересуют. _poll(2)_ может работать с теми же объектами, что и _select(2)_.  
...
Собственно, _суперблок_ считывается в оперативную память при монтировании файловой системы, и находится там до момента её отключения (_демонтирования_).  
...
Как мы упоминали в предыдущей главе про файлы, при использовании _lseek(2)_ в файле могут возникать дыры в виде нулевых байт. Эти дыры не храняться на физических блоках данных, поэтому соответствующие элементы массива адресов в _inode_ хранят нулевые указатели. Когда процесс производит чтение такого блока, ядро возвращает последовательность нулей.  
...
Эта ФС появилась в версии 4.2BSD UNIX. Ключевые изменения по сравнению с  _s5fs_ - это отделение списка свободных блоков данных и списка свободных _inode_ от _суперблока_, а также многократное резервное дублирование суперблока.  
...
Первые два элемента также, как и в _s5fs_ адресуют сам каталог и его родителя.  При удалении имени из каталога принадлежащая ему запись "присоединяется" к предыдущей, посредством увеличения поля _d_reclem_ на длину удаляемой записи. Данные записи более невалидны. Таким образом, в каталоге избегается появление "дыр".  

#### Виртуальная файловая система

Современные версии UNIX обеспечивают одновременную работу с несколькими типами файловых систем за счёт технологии _виртуальной файловой системы_. Взаимодействие с физической файловой системой осуществляется через неё, тем самым оно не зависит от конкретной реализации.  

Интерфейсом работы с файлом для любой файловой системы является _v_node_ (_virtual node_). Он имеет схожую семантику с традиционным _inode_ в UNIX. Метаданные всех активных файлов, т.е. тех, на которые ссылаются процессы представленны в памяти в виде _in-core idnode_, в качестве которых при наличии виртуальной ФС выступают _vnode_.

Независимо от типа физической ФС структура _vnode_ одинакова для всех файлов. _vnode_ содержит следующие данные:  
...
struct vnode {
     ...
     struct vfs *v_vfsmountedhere; /* Указатель на подключенную файловую
                                   систему, если vnode является точкой
                                   монтирования */
     struct vfs *v_vfsp; /* Указатель на физическую файловую систему */
     enum vtype v_type;  /* тип vnode: регулярный файл, каталог, специальный файл устройства, сокет и т.д. */
     caddr_t v_data;     /* Указатель на данные, относящиеся к реальной файловой системе */
     struct vnodeops *v_op;   /* список доступных операций для vnode */
}
```

### Межпроцессное взаимодействие

Напрямую процессы не могут обращаться к данным друг друга из-за концепции виртуального адресного пространства, которое ограничивает область доступных для процесса данных.  

Разумеется на практике возникают ситуации, когда приложения вынуждены общаться друг с другом: клиент-серверные приложения, приложения использующие результаты работы другого, приложения, использующие какие-то разделяемые ресурсы и т.д.

UNIX подобные ОС предоставляют механизмы для межпроцессного взаимодействия, используя которые, процессы могут обмениваться данными друг с другом - сокеты, разделяемая память, сигналы, трубы (_pipes_), FIFO.
...
Сигналы появились ещё в ранних версиях UNIX, но были реализованы ненадёжно. Возникал ряд проблем: сигнал мог быть утерян, также имелись сложности блокирования выполнения сигналов на время работы с критическими секциями кода. В версии _System V_ были внесены изменения, предоставлявшие ___надёжные___ (_reliable_) сигналы. В настоящее время интерфейс надёжных сигналов регламентирует стандарт __POSIX.1__. В ранних версиях UNIX, если процессу посылались два сигнала одного типа и второй был отправлен до того, как первый был обработан, то второй сигнал просто терялся и подвергался обработке. _Надёжные_ сигналы, появившиеся позже, обеспечивали гарантированную доставку до процесса и последующую обработку, эти сигналы отличаются от обычных как по процедуре генерации, так и по процедуре обработки. В рамках текущего курса такие сигналы не изучаются.  
...
Хоть и програмно сигналы могут отправляться от процесса к процессу, на самом деле они проходят через ядро, что очевидно, поскольку функции отправки сигналов такие, как _kill(2)_ и _sigsend(2)_ являются системными вызовами.  

Каждый процесс имеет _сигнальную маску_, наследуемую от родителя, которая определяет множество сигналов, получение которых заблокированно.  

Чтобы пересылать сигналы между потоками в рамках одного процесса, нужно использовать функцию _pthread_kill(3)_.  

__Получение сигнала__

Сигналы обрабатываются получающим процессом. Даже при выполнении действий по умолчанию, когда при обработки сигнала никакой пользовательский код не вызывается, это происходит в контексте процесса.  

Доставленные сигналы прерывают некоторые системные вызовы такие, как _wait(2)_ и _read(2)_. Если сигнал не привёл к завершению процесса, то прерванные функции, как правило, завершаются с ошибкой.  

Если реакция на сигнал у процесса не установленна, то процесс выполняет реакцию по умолчанию. В Unix-системах бывает 4 типа реации по умолчанию:  

* __exit__ - процесс, получивший сигнал, завершается. В его состоянии указывается, что он был завершён по сигналу.  

* __core__ - процесс завершается, как и при реацкции __exit__, кроме того, в текущей дирректории создаётся _core-файл_, который содержит записи состояния процесса на момент прерывания. Такая реакция происходит в ситуациях, когда пользователь написал некорретный код и в процессе исполнения была создана исключительная ситуация. Цель core-файла предоставить пользователю возможность отладки, чтобы он смог определить, где возникла исключительная ситуация.  

* __stop__ - процесс останавливает работу.  

* __ignore__ - процесс игнорирует сигнал, как если бы он не возникал вовсе.  

Когда функция обработки сигнала завершается, процесс возобновляет работу с того места, где он был прерван, если, конечно, функция обработки не сделала других распоряжений.  

Приведём несколько примеров сигналов:  

|сигнал|реакция|событие|
|---|---|---|
|SIGQUIT|core|завершение (см. termio(7))|
|SIGABORT|core|аборт|
|SIGKILL|exit|принудительное завершение процесса|
|SIGSEGV|core|ошибка сегментации|
|SIGTERM|exit|програмный сигнал прерывания от _kill(2)_|
|SIGSTOP|stop|остановка процесса (сигналом)|
|SIGTSTP|stop|остановка процесса (пользоваателем)|
|SIGCONT|ignore|продолжение исполнение процесса|
|SIGTTIN|stop|ожидание ввода с терминала|
|SIGTTOU|stop|ожидание вывода на терминал|

__Установка реакции на сигнал__

Системные вызовы signal(2) устанавливает реакцию на сигнал, т.е действия, которые процесс предпринимает, когда получает заданный сигнал.  
...
Функция устанавливает диспозицию сигнала с номером _signum_ в значение _handler_, который может являться либо _SIG_IGN_ (проигнорировать сигнал), _SIG_DFL_ (установка реакции по умолчанию), либо адресс функции обработчика, определённой программистом. Поведение _signal(2)_  неопределено в многопоточном процессе.  

Пример реализации обработки перехваченного сигнала:  

```C
#include <stdio.h>
#include <signal.h>
#define OUTPUT "Primes"
#define MAXNUM 10000
...
Сигнальная маска - это объект типа _sigset_t_, который представляет из себя битовую маску сигналов. Если бит, соответствующий сигналу установлен, то сигнал присутствует в маске, в противном случае сигнал в маске отсутствует.  

Поскольку _sigset_t_ непрозрачный тип, то существует набор функций для манипуляций с маской сигналов:  
...
* _sigemptyset(3C)_ - инициализация маски по указателю _set_ пустым множеством сигналов.  
* _sigfillset(3C)_ - инициализация полным множеством сигналов.  
* _sigaddset(3C)_ - добавление сигнала _signum_ в маску.  
* _sigdelset(3C)_ - удаление сигнала _signum_ из маски.  
* _sigismember(3C)_ - проверяет есть ли сигнал _signum_ в маске.  

Перед выполнением операций модификации маски _sigset_t_, она должна быть проинициализированна _sigfillset(3С)_, или _sigemptyset(3С)_.  

__Изменение маски сигналов процесса__

Маска сигналов процесса - это атрибут процесса, хранящийся в его _user-space_. Сигналы, включённые маску являются заблокированными для процесса. Они не игнорируются, но и не доставляются процессу до тех пор, пока не будут разблокированны (т.е. исключены из маски).  

Сигнальная маска наследуется при _fork(2)_ и _exec(2)_. Для изменения сигнальной маски процесса предусмотрен системный вызов _sigprocmask(2)_.
...
```
Механизм установки маски процесса определяется флагом _how_, который может принимать одно из следующих значений:  

* __SIG_BLOCK__ - набор сигналов в результирующей маски - это объединение множеств сигналов из нового маски (_set_) и из старой сигнальной маски процесса.  

* __SIG_UNBLOCK__ - из текущей маски сигналов убираются сигналы из новой маски.  

* __SIG_SETMASK__ - маска сигналов процесса устанавливается в маску, на которую указывает _set_.  

Если значение _oldset_ не _NULL_, то в область памяти, на которую он указывает, помещается старая сигнальная маска процесса.  

Если значение _set_ рано _NULL_, то маска сигналов процесса не меняется. Т.е. функцию можно использовать просто для получения значения текущей маски.  

Использование данной функции не специфицированно в многопоточной среде. Для той же цели в многопоточной среде нужно использовать _pthread_sigmask(3)_. Поток наследует маску сигналов своего создателя (такая опция включена в _clone(2)_ через который в Linux и реализовано создание нового потока в рамках одного процесса.)

#### Pipe

_Трубы_ или "пайпы" (_pipes_) представляют из себя первоначально однонаправленные (_симплексные_) каналы передачи данных между РОДСТВЕННЫМИ процессами (например между родительским и дочерним процессами). Дело в том, что семантика _pipe_ такова, что только родственные процессы могут получить к нему доступ. Поскольку работа с неименованными каналами осуществляется только через файловые дескрипторы, то только родственному процессу можно их предоставить, поскольку он наследует номера дескрипторов от родительского. В UNIX SVR4 каналы стали двунаправленным (_дуплексным_) механизмом передачи данных, так что, на некоторых системах, два процесса могут обмениваться данными в обоих направлениях, используя одну и ту же трубу. К сожалению (или к счастью) Linux не входит в их число и работа с каналами в нём симплексная.  

Обычные трубы не имеют имени, поэтому они также называются _анонимными каналами_. Данные в них передаются в порядке _FIFO_. При передачи данных через трубы, в рамках стандартной реализации, используется дисковая буферизация. Т.е. обмен данными через "пайпы" связан через дисковый ввод/вывод, что имеет соответствующие издержки в производительности.  

Современные ОС используют более эффективные механизмы реализации передачи данных через каналы. Например, SCO UNIX использует для этого специальную файловую систему HPPS (High Performance Pipe System), которая буфферезирует данные в оперативной памяти.  

Трубы не имеют ограничений на объём передаваемых данных (в отличие от обычных файлов), хоть и имеют сравнительно небольшой внутренний буфер. Размер этого буфера можно посмотреть при помощи системного вызова _pathconf(2)_. В _Solaris 10_ он вернёт значение 510 (в байтах), на самом деле это число соответсвует объёму данных, которые гарантированно могут быть полностью записаны посредством одного вызова _write(2)_. На практике, размер буффера трубы приблизительно составляет 100 килобайт. Ещё одно важное отличие каналов от регулярных файлов это то, что чтение данных из канала разрушает данные в нём, т.е. повторно те же данные из канала получить невозможно.  

Также, трубам не нужно заботиться о переполнении канала или о невозможности чтения из пустого буффера. Если буффер заполнен, то пишущий туда процесс заблокируется до тех пор, пока процесс на другом конце не осуществит чтение данных из трубы. Аналогично, читающий процесс блокируется, когда пытается читать из пустого буффера. Это реализация механизма синхронизации канальной передачи данных.  

Существуют также _именованные каналы_ или _FIFO_. В отличии от обычных труб для них создаются отдельные именованные записи в файле. Главное преимущество _именованных каналов_ над обычными каналами заключается в том, что они могут быть использованы для комуникации неродственных процессов.  

Именованные каналы существуют, пока существуют их соответствующие файлы. Обычные трубы же существуют до тех пор, пока существуют процессы, использующие их.  

Каналы можно использовать для неблокирующих чтения/записи при мультиплексировании, посредством установки флага _O_NONBLOCK_. В таком случае, автоматический механизм синхронизации у канала отключится.  

__Доступ к данным в канале__

Концы каналов идентифицируются обычными файловыми дескрипторами. Большинство системных вызовов, работающих с файловыми дескрипторами, применимы и к каналам.  

Отметим, что каналы используются командными интерпретаторами для реализации конвейеров (_pipeline_). При создании конвейера стандартный вывод одного процесса передаётся на стандартный вход другого через трубу.  

Запись данных в канал осуществляется также, как и в регулярный файл - посредством системного вызова _write(2)_. Однако, ОС не допускает частичной записи в _pipe_: _write(2)_ блокируется до тех пор, пока все данные не будут записаны, либо до момента возникновения ошибки. Если процесс пытается писать в канал, из которого никто не читает, он получит сигнал SIGPIPE. Ядро поддерживает механизм атомарной записи в канал, т.е. если в один канал пишут несколько процессов, то записи будут упорядоченны и никакой потери данных не будет.  

Чтение данных из канала осуществляется системным вызовом _read(2)_ соответственно. Поскольку, чтение разрушает данные в канале, использовать _lseek(2)_ для того, чтобы прочитать данные заново не получится.  

#### Сокеты

Сокеты в некоторых моментах схожи с именованными каналами. Они также имеют запись в файловой системе, также могут быть использованны для комуникации неродственных процессов и работа с ними осуществляется через файловый дескриптор, который можно использовать при операциях _read(2)_, _write(2)_, _select(2)_/_poll(2)_. Однако, они имеют ряд довольно существенных отличий. Сокеты работают в _дуплексном_ режиме, т.е. поддерживают независимую двустороннюю передачу данных. ОС не синхронизирует запись и чтение из сокета на его разных концах, в отличие от труб. Также, сокеты можно использовать для межсетевой коммуникации процессов, т.е. для общения приложений по сети, которые работают на разных устройствах. Трубу же для таких целей использовать не получится, поскольку оба процесса должны иметь доступ к её файлу, а если процессы работают на разных устройствах, то, очевидно, такое невозможно.  

В главе ___Библиотечные ф-ции сетевых сервисов___ описывается _API_ сокетов Unix.  

### Разделяемая память

Разделяемая память - наиболее быстрый способ межпроцессной комуникации. Разделяемая память становится частью виртуального адресного пространства каждого процесса, который её разделил, при этом область виртуальной памяти, которая ей соответствует отображается на одну и ту же физическую память для каждого процесса. Таким образом, несколько процессов могут разделять одни и те же физические данные.  

Того же эффекта можно достичь, используя _mmap(2)_ с флагом _MAP_SHARED_ для доступного на запись файла. Однако, использование _mmap(2)_ имеет практическое отличие - этой ф-ции нужен файл, который она будет отображать, а разделяемая память ни к какому файлу не привязана. Кроме того, система синхронизирует содержимое отображённой памяти с данными отображённого файла на диске, что может быть нужно для каких-то целей, однако это имеет соответствующие издержки в производительности.  
...
В рамках процесса, такой подход реализует псевдо-многопоточность, поскольку в процессе в данный момент времени исполняется только одна нить.  
...
1. Нить является подчинённой, по отношению к процессу сущностью. Идентификатор нити состоит из идентификатора процесса и собственного идентификатора нити, уникального в рамках данного процесса. Большинство UNIX-систем использует именно такой подход, в т.ч. Solaris 10 и Windows.  

2. Нить является сущностью того же уровня, что и процесс. В рамках данного подхода, нити - это те же процессы в т.ч. и с точки зрения ядра ОС. Но эти процессы отличаются от тех, которые образуются при вызове _fork(2)_ - они совместно используют общее адресное пространство с другими процессами. Данный подход реализован в Linux.  

В Linux, у процесса есть дополнительный атрибут - _tid_ (_thread id_) идентификатор потока. В Linux поток - это тоже процесс, который разделяет виртуальное адресное пространство с другими потоками, концептуально в рамках "своего" процесса. Поэтому, _pid_ у потоков одного процесса одинаковый, но _tid_ - разный.  В случае, если процесс однопоточный, то его _tid_ равен _pid_.  

Поскольку ОС реализующие многопоточность через системный планировщик (в частности Solaris 10 и Linux) используют именно системные нити, то каждому потоку POSIX Thread API соответствует отдельный LWP, но, в таком случае, планирование нескольких LWP может осуществляться на одном потоке ядра ПРОЦЕССОРА. LWP - по сути является интерфейсом взаимодействия пользовательских потоков с потоками ядра.  

__гибридная реализация__



__ядерные потоки, пользовательские потоки, процессор__

При рассмотрениии концепций испольнения процессов, потоков и того, как это связано с аппаратным планированием на процессоре возникает некоторая путанница. Для понимания, обозначим концепции _пользовательского потока_, _ядерного потока_ и опишем механизм их исполнения более подробно.  

_Пользовательские потоки_ - это отдельные структуры хранящиеся в _user-space_ процесса, которому они принадлежат. Эти структуры предоставляют интерфейс управляющих данных для единицы планирования в коде пользователя. Таких потоков может быть много и их исполнение будет казаться пользователю параллельным, не зависимо от того, как этим потокам передаётся управление в рамках реализации в ОС.  

_Системные потоки_ - или ядерные потоки, представляют из себя управляющие структуры единиц планирования на уровне ядра ОС т.е. это информация для планирования конкретных задач ОС.  

Процессор осуществляет исполнение ядереных потоков. Поскольку число ядрер процессора и, следовательно число аппаратных потоков ограничено, то для того, чтобы правильно осуществлять планирование аппаратного потока на исполнение конкретной системной нити существует _системный планировщик_, который эту задачу и выполняет. Не путать __поток ядра__ ОС и __аппаратный поток__ ядра процессора!  

Механизм планирования конкретной пользовательской нити на системной реализуется в рамках одного из вышеописанных подходов: используя _системный планировщик_, _пользовательский планировщик_ и _гибридную реализацию_.  

__Переключение контекста__

* ___Переключение контекста___ - это прекращение процессором выполнения текущей задачи (задачи может соответствовать процесс или поток) с сохранением всей управляющей информации в ядре ОС для возможного продолжения выполнения этой задачи с того же места, с последующей загрузкой управляющей информации из контекста новой задачи, которая соответствует её последнему актуальному состоянию. После, происходит исполнение процессором новой задачи.  

В рамках реализаций многопоточности, которые мы рассмотрели в том или ином виде присутствуют системные нити. Если переключение контекста происходит от одной нити к другой, которые соответствуют разным процессам, т.е. файктически происходит переключение контекста процесса, то тогда в процессоре обновляется вся управляющая информация - полностью переключается информация о виртуальном адресном пространстве, таблице трансляций.  

В случае, если происходит переключения контекста с одной нити на другую в рамках одного процесса, то в таком случае виртуальное адресное пространство остаётся то же, и управляющая информация процессора меняется незначительно. Таким образом переключение контекста между потоками намного быстрее, чем между процессами. В случае с пользовательским планировщиком, переключение контекста между пользовательскими потоками вообще не осуществляет какого либо взаимодействия с ресурсами ядра вне пользовательской области своего процесса. Таким образом, переключение контекста между нитями одного процесса в реализации с пользовательским планировщиком происходит практически мнгновенно.  

__Создание потока в Linux__

Поскольку поток это системный ресурс, то для его создания существует соответствующие системный вызов. Как мы значем, для создания нового процесса предусмотрен вызов _fork(2)_, но он создаёт процесс, который не разделяет адресное пространство родительского процесса, а имеет своё собственное (концептуально, это именно так, но реализация этого более сложная см. "Создание процесса").  

Поток в Linux это тот же процесс, но который полностью разделяет адресное пространство с другими. Для создание такого процесса в Linux существует системный вызов _clone(2)_. Он предоставляет множество настроек для указания конкретных сегментов виртуального адресного пространства процесса, которые будут разделяемыми и которые разделяемыми не будут. Можно сделать так, что новый процесс будет полностью разделять адресное пространство "родительского", тогда это будет новый поток исполнения. Можно сделать так, чтобы процесс вовсе не имел разделяемого пространства с вызывающим родителем, тогда это будет _контейнер_.  
...
Данный системный вызов предоставляет более точный контроль за тем, какие сегменты виртуального адресного пространства или структуры родительского процесса будут разделяться с дочерним.  

Например, можно контролировать будут ли процессы разделять виртуальное адресное пространство, будут ли совместно использовать таблицу файловых дескрипторов и таблицу обработчиков сигналов.  

После создания дочернего процесса вызововм _clone(2)_ он начинает исполнение функции _fn_, передавая ей в качестве аргумента _arg_. Когда ф-ция _fn_ возвращает управление, дочерний процесс завершается. Дочерний процесс также может завершиться явно вызвав _exit(2)_ или получив фатальный сигнал.  

Аргумент _stack_ указывает на начальный адресс стека нового процесса. Поскольку родительский и дочерний процессы могут разделять стек, то нужно аккуратно выделить в нём память под стек дочернего процесса, так чтобы не было коллизий. Этим должен озаботиться родительский процесс аккуратно выделить память под стек и передать адрес его начала в _clone(2)_.  

_flags_ - битовая маска флагов, которая специфицирует что будет разделяться между родительским и дочерним процессами. Маска составляется посредством поразрядного _ИЛИ_ между следующими значениями:  

* __CLONE_PARENT__ - указать для нового процесса тот же родительский процесс, что и у процесса, вызвавшего _clone(2)_.  
<br/>

* __CLONE_SIGHAND__ - при указании данного флага и родительский и дочерний процес полностью разделяют таблицу обработчиков сигналов.  
<br/>

* __CLONE_FILES__ - дочерний процесс и родитель полностью разделяют таблицу файловых дескрипторов. Если данный флаг не указан, то дочерний процесс копирует таблицу файловых дескрипторов у родителя и работает с ней независимо, в таком случае любые действия по изменению состояний файловых дескрипторов у родителя или у потомка не окажут друг на друга влияния.  
<br/>

* __CLONE_THREAD__ - если указан данный флаг, то дочерний процесс имеет ту же группу потоков, что и процесс, вызвавший _clone(2)_. Дочерний процесс имеет тот же идентификатор процесса, что и родитель и тот же родительский процесс, что и родитель. Т.е. атрибуты _pid_ и _ppid_ у них совпадают. Если же этот флаг не указан, то созданный процесс работает в рамках новой группы потоков, в которой он становится лидером.  
<br/>

* __CLONE_VM__ - если данный флаг указан, то оба процесса полностью разделяют виртуальное адресное пространство между собой.  

__Механизмы реализации пользовательских потоков в Linux__

Теперь рассмотрим механизм создания собственных пользовательских потоков. По сути, нужно создать структуру, описывающую наш собственный контекст исполнения для процесса, а что такое контекст? Минимальное, что он должен в себя включать это стек и исполняемую фунцию. Этого будет достаточно. Также, нужно заиспользовать какую-то ф-цию из API ОС чтобы передать этот контекст нашему процессу.  

Посредством создания нескольких таких контекстов, каждый со своей функцией и стеком и реализовав алгоритм его переключения в нашем процессе, мы реализуем свой пользовательский планировщик со своими пользовательскими потоками.  

В ОС Linux для этих целей существует несколько библиотечных функций:  
_getcontext(3)_,  _setcontext(3)_,  _makecontext(3)_, _swapcontext(3)_. Эти функции позволяют переключать контекст исполнения на пользовательском уровне между потоками процесса.  

Рассмотрим структуру, описывающую пользовательский контекст, она включает в себя, как минимум, следующие поля:  
...
_uc_link_ - хранит указатель на контекст, который будет следующим, после того, как текущий контекст уничтожится.  

_uc_sigmask_ - множество заблокированных в данном контексте сигналов.  

_uc_stack_ - стек, который использует данный контекст.  

_uc_mcontext_ - аппаратно зависиммое представление сохранённого контекста.  

* __setcontext(3)__ - получение текущего пользовательского контекста.  
...
```
Данная функция инициализирует структуру, на котоую ссылается _ucp_, текущим контекстом.  

* __setcontext(3)__ - установка пользовательского контекста.  
...
Восстанавливает пользовательский контекст значением, на которое указывает _ucp_. В случае успеха, функция не возвращает управление. Устанавливаемый контекст должен быть получен посредством вызова _makecontext(3)_, либо из результат _getcontext(3)_, либо получен в качестве аргумента обработчика сигнала.  

* __makecontext(3)__ - модификация текущего контекста.
...
Функция модифицирует контекст, на который указывает _ucp_ (который должен был быть получен посредством вызова _getcontext(3)_). Перед вызовом _makecontext(3)_ требуется аллоцировать новый стек под контекст, и проинициализировать соответствующее поле в _ucp_: _ucp->uc_stack_. Также необходимо определить последующий контекст и также проинициализировать соответствующее поле в _ucp_: _ucp->uc_link_.  

Когда новый контекст будет активирован, посредством вызова _setcontext(3)_ или _swapcontext(3)_, будет вызвана функция _func_ с указанными аргументами по числу _argc_.  

Если указатель контекста приеемника равен _NULL_, то процесс (поток) завершиться.  

* __swapcontext(3)__ - переключение контекстов.  
...
Функция сохряняет текущий контекст по указателю _oucp_, затем, активирует контекст по указателю _ucp_. В случае успеха, _swapcontext(3)_ не возвращает управление, однако он может это сделать, если контекст будет переключен обратно на указываемый _oucp_.  

Инициализация нового контекста может выглядеть так:  

``` C
     static ucontext_t uctx_func1, uctx_main;
