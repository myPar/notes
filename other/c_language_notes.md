...
* __Определение__ - _определение_ (или _реализация_) переменной или ф-ции подразумевает, что к тому, что уже делает _объявление_, оно дополнительно указывает компилятору на аллоцирование памяти для этой переменной или ф-ции и на место. В случае переменой, также указывается где эта память будет аллоцирована.  
...
<br/>
    ...
    __использование:__
    _extern_ используется для обеспечения ссылочности на определения глобальных переменных или ф-ций из другого файла программы. Как это используется - в заголовочном файле, где мы планируем объявить наши глобальные переменные и ф-ции мы делаем это, используя ключевое слово _extern_. В одном исходном _c_-файле мы подключаем данный заголовочный файл и реализуем их. В другом исходном _с_-файле, мы можем эту реализацию использовать, подключив предварительно вышеупомянутый заголовок.  
...
* __fgets()__ - ф-ция читает байты из указанного потока и помещает их в буффер.  
...
Ф-ция читает байты из указанного потока _stream_ в буффер _s_ до тех пор, пока n-1 байт не будет прочитано, либо до тех по, пока не будет прочитан символ перевода строки (также помещается в буффер), либо, пока не будет достигнуто условие достижения конца файла. После этого в конец буффера помещается символ '\0'.  

В случае, если размер буфера _s_ превышает длину считываемой строки поведение в неопределено.  

__возвращаемые значения:__

Если был достигнут конец файла и символов прочитано не было, то в буффер не записываются никакие байты и ф-ция возвращает NULL. Если для потока установлен индикатор конца файла, то поведение аналогично вышеописанному случаю. Если в процессе чтения возникает ошибка, то ф-ция возвращает NULL и для потока устанавливается __индикатор ошибки__. Если, в процессе чтения достигается конец файла, то для потока устанавливается индикатор EOF (про возвращаемое значение в этом случае в документации не сказано ничего). В противном случае, возвращается _s_.  

__ошибки:__

---

* __ferror()__ - определяет произошла ли ошибка в процессе чтения/записи в указанном потоке.  
...
Ф-ция возвращает ненулевое значение, если ранее в процессе чтения/записи в указанном потоке _stream_ произошла ошибка. В противном случае возвращается 0. [Документация.](https://docs.oracle.com/cd/E26505_01/html/816-5168/ferror-3c.html#scrolltoc)  

---

* __feof()__ - определяет был ли для указанного потока установлен индикатор EOF. 
...
Ф-ция возвращает ненулевое значение, если ранее, в процессе чтения из потока _stream_ для него был установлен индикатор EOF. В противном случае возвращается 0.  
