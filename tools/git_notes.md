...
$ git rebase -i dst_branch

### интерактивный rebase ветки без её перемещения
$ git rebase -i @~[commits_count]
```
...
f - fixup: 
```
Произведение указанных в файле изменений будет осуществляться последовательно в процессе работы rebase. Оригинальный порядок коммитов указывается сверху вниз. Перед коммитом с указанной опцией squash должен иметься хотя бы ещё один коммит.  

Указываем опцию squash в двух последних коммитов. Результирующее дерево коммитов после интерактивного rebase будет выглядеть так:  

``` bash
* commit 1194aabf3ee38eaf16033c5feeb6b0a468f2cda3 (HEAD -> feature_branch)
| Author: myPar <andrebobrovs@mail.ru>
| Date:   Sat Feb 25 11:26:48 2023 +0700
|
|     squashed commit in feature branch
|
* commit 54ba37b98ea70348c0dbeb1fb671dd47871b47e0 (master)
| Author: myPar <andrebobrovs@mail.ru>
| Date:   Sat Feb 25 11:29:01 2023 +0700
...
```
Как видно из вывода, три коммита в ветке feature_branch схлопнулись в один.  
